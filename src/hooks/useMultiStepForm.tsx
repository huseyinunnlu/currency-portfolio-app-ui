...
import { FieldValues, UseFormReturn, Path, UseFormProps, useForm } from 'react-hook-form'
...
export default function useMultiStepForm<FormTypes extends FieldValues>({
    ...
    useEffect(() => {
        if (formProps.mode === "onChange") {
            form.watch(() => {
                const isValidStep = form.trigger(fieldsByStep[activeStep] as Path<FormTypes>[])
                if (!isValidStep) {
                    setIsValid(false)
                }
            })
    ...
    const nextStep = async () => {
        const isValidStep = await form.trigger(fieldsByStep[activeStep] as Path<FormTypes>[])
        if (!isValidStep) {
            return
        }
    ...
    return { activeStep, nextStep, prevStep, isValid, form, stepCount: fieldsByStep.length }
}
